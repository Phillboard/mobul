...
const STEP_SCRIPT_MAPPING: Record<WorkflowStep, string[]> = {
  ...
  optin: ["verification", "explanation"],
  contact: ["explanation"],
...
export function ScriptPanel({
  ...
  // Auto-expand when scripts become available, collapse when empty
  useEffect(() => {
  ...
  const hasScripts = relevantScripts.length > 0;
      ...
      <Card className="border-0 shadow-sm bg-slate-50/50">
        <CollapsibleTrigger asChild>
          <button className="w-full">
            <CardHeader className="py-3 px-4 flex flex-row items-center justify-between hover:bg-slate-100/50 transition-colors rounded-t-lg">
              <CardTitle className="text-sm font-medium flex items-center gap-2 text-muted-foreground">
                <FileText className="h-4 w-4" />
                Call Scripts
                {hasScripts && (
                  <Badge variant="secondary" className="text-xs ml-2">
                    {relevantScripts.length} available
                  </Badge>
                )}
              </CardTitle>
              {isPanelOpen ? (
          ...
          <CardContent className="pt-0 pb-3 px-4 space-y-2">
            {!hasScripts ? (
              <div className="text-sm text-muted-foreground py-2">
                No scripts available for this step
              </div>
            ) : (
              relevantScripts.map((script) => {
                ...
                return (
                  <div key={script.id} className="bg-white rounded-lg border border-slate-200">
                    <button
                    ...
                    {isExpanded && (
                      <div className="px-3 pb-3 space-y-2">
                        <div className="text-sm whitespace-pre-wrap text-muted-foreground border-l-2 border-primary/50 pl-3 bg-slate-50 p-2 rounded">
