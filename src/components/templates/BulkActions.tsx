...
export function BulkActions({ selectedIds, templates, onClearSelection }: BulkActionsProps) {
  ...
  const bulkFavoriteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`${selectedIds.length} templates marked as favorite`);
      onClearSelection();
    },
    onError: () => {
  ...
  const bulkDuplicateMutation = useMutation({
    mutationFn: async () => {
      const templatesToDuplicate = templates.filter((t) => selectedIds.includes(t.id));
      
      const duplicates = templatesToDuplicate.map((t) => ({
        name: `${t.name} (Copy)`,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["templates"] });
      toast.success(`${selectedIds.length} templates duplicated`);
      onClearSelection();
    },
    onError: () => {
