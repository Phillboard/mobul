-- ============================================
-- COMPREHENSIVE RLS AUTH FUNCTION FIX v2
-- Handles PostgreSQL's quoted identifier format: "auth"."uid"()
-- ============================================
...
BEGIN
  RAISE NOTICE '=== Starting Comprehensive RLS Auth Function Fix v2 ===';
  
  -- Loop through all RLS policies in the public schema that need fixing
  -- Now handles the quoted identifier format: "auth"."uid"()
  FOR policy_record IN
    ...
    WHERE n.nspname = 'public'
      AND (
        -- Find policies with "auth"."uid"() not wrapped (quoted format)
        ...
        OR
        -- Also check unquoted format just in case
        (pg_get_expr(pol.polqual, pol.polrelid) ~ 'auth\.uid\(\)'
         AND pg_get_expr(pol.polqual, pol.polrelid) !~ '\(SELECT auth\.uid\(\)\)')
        OR
        (pg_get_expr(pol.polwithcheck, pol.polrelid) ~ 'auth\.uid\(\)'
  ...
  LOOP
    BEGIN
      ...
      -- Replace quoted "auth"."uid"() with (SELECT "auth"."uid"())
      IF new_qual IS NOT NULL THEN
        -- Handle quoted format "auth"."uid"() - in various contexts
        -- = "auth"."uid"()
        new_qual := regexp_replace(new_qual, '= "auth"\."uid"\(\)', '= (SELECT "auth"."uid"())', 'g');
        -- "auth"."uid"() = (when uid is on left side)
        new_qual := regexp_replace(new_qual, '"auth"\."uid"\(\) =', '(SELECT "auth"."uid"()) =', 'g');
        -- Inside function calls like has_role("auth"."uid"(), ...)
        new_qual := regexp_replace(new_qual, '"public"\."has_role"\("auth"\."uid"\(\)', '"public"."has_role"((SELECT "auth"."uid"())', 'g');
        new_qual := regexp_replace(new_qual, '"public"\."user_can_access_client"\("auth"\."uid"\(\)', '"public"."user_can_access_client"((SELECT "auth"."uid"())', 'g');
        new_qual := regexp_replace(new_qual, '"public"\."get_user_org_ids"\("auth"\."uid"\(\)', '"public"."get_user_org_ids"((SELECT "auth"."uid"())', 'g');
        new_qual := regexp_replace(new_qual, '"public"\."get_user_client_ids"\("auth"\."uid"\(\)', '"public"."get_user_client_ids"((SELECT "auth"."uid"())', 'g');
        -- Handle "auth"."jwt"()
        new_qual := regexp_replace(new_qual, '"auth"\."jwt"\(\)', '(SELECT "auth"."jwt"())', 'g');
        -- Catch remaining "auth"."uid"() that haven't been wrapped yet
        new_qual := regexp_replace(new_qual, '([^SELECT] )"auth"\."uid"\(\)', '\1(SELECT "auth"."uid"())', 'g');
        
        -- Handle unquoted format as well
        new_qual := regexp_replace(new_qual, '= auth\.uid\(\)', '= (SELECT auth.uid())', 'g');
      ...
      IF new_with_check IS NOT NULL THEN
        ...
        new_with_check := regexp_replace(new_with_check, '"auth"\."uid"\(\) =', '(SELECT "auth"."uid"()) =', 'g');
        new_with_check := regexp_replace(new_with_check, '"public"\."has_role"\("auth"\."uid"\(\)', '"public"."has_role"((SELECT "auth"."uid"())', 'g');
        new_with_check := regexp_replace(new_with_check, '"public"\."user_can_access_client"\("auth"\."uid"\(\)', '"public"."user_can_access_client"((SELECT "auth"."uid"())', 'g');
        new_with_check := regexp_replace(new_with_check, '"public"\."get_user_org_ids"\("auth"\."uid"\(\)', '"public"."get_user_org_ids"((SELECT "auth"."uid"())', 'g');
        new_with_check := regexp_replace(new_with_check, '"public"\."get_user_client_ids"\("auth"\."uid"\(\)', '"public"."get_user_client_ids"((SELECT "auth"."uid"())', 'g');
  ...
  RAISE NOTICE '=== RLS Auth Fix v2 Complete ===';
